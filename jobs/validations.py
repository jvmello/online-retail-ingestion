...
def assert_columns_exist(
    df: DataFrame, expected_columns: list[str], df_name: str
) -> None:
...
def assert_non_negative(df: DataFrame, columns: list[str], df_name: str) -> None:
    for column_name in columns:
        ...
        if invalid_count > 0:
            raise ValueError(
                f"{df_name}.{column_name} has {invalid_count} negative values"
            )
...
def assert_row_count_above(df: DataFrame, minimum: int, df_name: str) -> None:
    ...
    if row_count < minimum:
        raise ValueError(
            f"{df_name} has only {row_count} rows, expected at least {minimum}"
        )
